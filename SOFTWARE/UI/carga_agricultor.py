...
from INFRA.openrouter_service import consultar_chatbot
import os
import cv2
import time
import math
...
class ChatWorker(QThread):
    ...
    def __init__(self, pregunta, contexto):
        super().__init__()
        self.pregunta = pregunta
        self.contexto = contexto

    def run(self):
        resp = consultar_chatbot(self.pregunta, self.contexto)
        self.respuesta_lista.emit(resp)
...
class AnalisisChatWindow(QWidget):
    ...
    def enviar_pregunta(self):
        ...
        self.chat_area.append("<i>AgroScan está escribiendo...</i>")

        # Preparar contexto
        contexto = {
            "sanos": self.aptos,
            "enfermos": self.no_aptos,
            "total": self.total_detectados,
            "resumen_texto": self.resumir_resultados_yolo(self.yolo_detections)
        }
        
        # Iniciar worker
        self.worker = ChatWorker(pregunta, contexto)
        self.worker.respuesta_lista.connect(self.mostrar_respuesta_ia)
        self.worker.start()

    def mostrar_respuesta_ia(self, respuesta):
        # Borrar el mensaje de "escribiendo..." (truco simple: borrar última línea si se pudiera, pero append está bien)
        # O mejor, simplemente agregar la respuesta.
